...
class StepLinear(ABCStep):
    ...
    def iterate(self, Q2):
        ...
        self.converged = (np.abs(self.norm_grad).max() < self.tol)
        return Q2

    def tmle_step(self, Q2):

        t1_slice = np.apply_along_axis(lambda f: quick_simpson(f, self.v_grid), 1, self.T1 * Q2 + self.T1 * Q2)
        t2_slice = np.apply_along_axis(lambda f: quick_simpson(f, self.v_grid), 1, self.T2 * Q2 + self.T2 * Q2)
